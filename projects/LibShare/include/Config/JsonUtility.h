...
bool GetObjectSubValue(
    ...
    const TValue & defaultValue = TValue{})
{
    typename RAPIDJSON_NAMESPACE::GenericObject<Const, TJsonValue>::ValueType key(RAPIDJSON_NAMESPACE::StringRef(pKey));
...
bool GetObjectSubValue(
    const RAPIDJSON_NAMESPACE::GenericObject<Const, TJsonValue> & object,
    const std::basic_string<typename RAPIDJSON_NAMESPACE::GenericObject<Const, TJsonValue>::Ch> & key,
    TValue & value,
    const TValue & defaultValue = TValue{})
{
...
namespace detail
{
    ...
    bool ParseImpl(TDocType & doc, RAPIDJSON_NAMESPACE::EncodedInputStream<TEncoding, TInputByteStream> & encodedInStream)
    {
        doc.template ParseStream<RAPIDJSON_NAMESPACE::ParseFlag::kParseDefaultFlags, TEncoding>(encodedInStream);
        ...
        bool prettyWrite)
    {
        if (prettyWrite)
        {
            RAPIDJSON_NAMESPACE::PrettyWriter<
                std::remove_reference_t<decltype(encodedOutStream)>,
    ...
    const boost::filesystem::path & file)
{
    ...
    RAPIDJSON_NAMESPACE::EncodedInputStream<RAPIDJSON_NAMESPACE::UTF8<>, decltype(inStream)> encodedInStream{ inStream };
...
bool ParseUtf8Memory(
    ...
    size_t nLength)
{
    RAPIDJSON_NAMESPACE::MemoryStream inStream{ (const RAPIDJSON_NAMESPACE::MemoryStream::Ch*)pData, nLength };
    RAPIDJSON_NAMESPACE::EncodedInputStream<RAPIDJSON_NAMESPACE::UTF8<>, decltype(inStream)> encodedInStream{ inStream };
    return detail::ParseImpl(doc, encodedInStream);
}
...
*/
template<typename TDocEncoding, typename TAllocator, typename TStackAllocator, 
         template<typename> class TInputStringStream, typename TStringEncoding>
bool ParseString(
    RAPIDJSON_NAMESPACE::GenericDocument<TDocEncoding, TAllocator, TStackAllocator> & doc, 
    TInputStringStream<TStringEncoding> & inputStrStream)
{
    doc.template ParseStream<RAPIDJSON_NAMESPACE::ParseFlag::kParseDefaultFlags, TStringEncoding>(inputStrStream);
    return !doc.HasParseError();
}
    ...
    bool putBOM = true)
{
    ...
    RAPIDJSON_NAMESPACE::EncodedOutputStream<RAPIDJSON_NAMESPACE::UTF8<>, decltype(outStream)> encodedOutStream{ outStream , putBOM };
...
{
    outBuffer.Clear();
    RAPIDJSON_NAMESPACE::EncodedOutputStream<RAPIDJSON_NAMESPACE::UTF8<>, std::remove_reference_t<decltype(outBuffer)> > encodedOutStream{ outBuffer, putBOM };
    return detail::WriteImpl(doc, encodedOutStream, prettyWrite);
}
...
*/
template<typename TDocEncoding, typename TAllocator, typename TStackAllocator, 
         typename TStrEncoding, typename TStrAllocator>
bool WriteToString(
    const RAPIDJSON_NAMESPACE::GenericDocument<TDocEncoding, TAllocator, TStackAllocator> & doc,
    ...
    bool prettyWrite = false)
{
    ...
    {
        RAPIDJSON_NAMESPACE::PrettyWriter<
            std::remove_reference_t<decltype(outString)>,
            TDocEncoding,
            TStrEncoding 
