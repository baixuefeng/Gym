...
public:
    ...
    {
        std::memcpy(this, &size, sizeof(size));
    }
...
public:
    ...
    {
        std::memcpy(this, &point, sizeof(point));
    }
    ...
    {
        return (std::abs(x - point.x) < FLT_EPSILON) &&
            (std::abs(y - point.y) < FLT_EPSILON);
    }
    ...
    CD2DPointF operator -() const
    {
        return CD2DPointF(-x, -y);
    }

    CD2DPointF operator +(CD2DPointF point) const
    {
        return CD2DPointF(x + point.x, y + point.y);
    }

    // Operators returning CD2DSizeF values
    CD2DSizeF operator -(CD2DPointF point) const
    {
        return CD2DSizeF(x - point.x, y - point.y);
    }
...
public:
    ...
    {
        return CD2DSizeF(right - left, bottom - top);
    }

    CD2DPointF& TopLeft()
    {
        return *((CD2DPointF*)this);
    }

    CD2DPointF& BottomRight()
    {
        return *((CD2DPointF*)this + 1);
    }

    const CD2DPointF& TopLeft() const
    {
        return *((CD2DPointF*)this);
    }

    const CD2DPointF& BottomRight() const
    {
        return *((CD2DPointF*)this + 1);
    }
    ...
    {
        return CD2DPointF((left + right) / 2, (top + bottom) / 2);
    }
    ...
    bool IsRectEmpty() const
    {
        return (left > right) 
            || (top > bottom)
            || (std::abs(left - right) < FLT_EPSILON)
            || (std::abs(top - bottom) < FLT_EPSILON);
    }

    bool IsRectNull() const
    {
        return (std::abs(left) < FLT_EPSILON)
            && (std::abs(top) < FLT_EPSILON)
            && (std::abs(right) < FLT_EPSILON)
            && (std::abs(bottom) < FLT_EPSILON);
    }
    ...
    void SetRectInfinite()
    {
        *this = D2D1::InfiniteRect();
    }
    ...
    void InflateRect(CD2DSizeF size)
    {
        InflateRect(size.width, size.height);
    }
    ...
    void DeflateRect(CD2DSizeF size)
    {
        InflateRect(-size.width, -size.height);
    }
    ...
    void OffsetRect(CD2DSizeF size)
    {
        OffsetRect(size.width, size.height);
    }

    void OffsetRect(CD2DPointF point)
    {
    ...
    void operator +=(const CD2DRectF & rect)
    {
        InflateRect(rect);
    }
