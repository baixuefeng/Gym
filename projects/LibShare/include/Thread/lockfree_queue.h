...
class lockfree_queue
{
    ...
    enum CONSTANT_VALUE : size_t
    {
        ...
        //每页的数据个数
        ITEMS_PER_PAGE =
        (sizeof(value_type) <= 4 ? 64 :
        sizeof(value_type) <= 8 ? 32 :
        sizeof(value_type) <= 16 ? 16 :
        sizeof(value_type) <= 32 ? 8 :
        sizeof(value_type) <= 64 ? 4 :
...
public:
    ...
    void push(T && ... data)
    {
        ...
        //如果需要换页，等待前一页处理完
        size_t nSpinCount = 0;
        while ((curList.m_pushCount.load(std::memory_order::memory_order_acquire) & PAGE_INDEX_MASK) != nPageIndex)
        {
            yield(nSpinCount++);
        }
        ...
        if (nPageOffset == ITEMS_PER_PAGE - 1)
        {
            ...
            nSpinCount = 0;
            while (curList.m_pushCount.load(std::memory_order::memory_order_acquire) != (index & SUBLIST_INDEX_MASK))
            {
                yield(nSpinCount++);
            }
    ...
    bool try_pop(value_type & data)
    {
        ...
        //如果需要换页，等待前一页处理完
        size_t nSpinCount = 0;
        while ((curList.m_popCount.load(std::memory_order::memory_order_acquire) & PAGE_INDEX_MASK) != nPageIndex)
        {
            yield(nSpinCount++);
        }
        ...
        if (nPageOffset == ITEMS_PER_PAGE - 1)
        {
            //最后一个处理者，换页，释放空闲页
            nSpinCount = 0;
            while (curList.m_popCount.load(std::memory_order::memory_order_acquire) != (index & SUBLIST_INDEX_MASK))
            {
                yield(nSpinCount++);
            }
