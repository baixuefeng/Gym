...
//-------------------------------------------------------------
    ...
    const_pointer address(const_reference tValue) const
    {
        return &tValue;
    }
    ...
    void deallocate(pointer pAddress, size_type /*uCount*/)
    {
        m_spAlloc->Free(pAddress);
    }

private:
    template<class _ValueType, class AllocType>friend class std_allocator_adaptor;

    std::shared_ptr<AllocType> m_spAlloc;
};

//不支持void
template<class T>class std_allocator_adaptor<void, T>;
template<class T>class std_allocator_adaptor<const void, T>;
template<class T>class std_allocator_adaptor<volatile void, T>;
template<class T>class std_allocator_adaptor<const volatile void, T>;

template<class _Ty, class _Other, class _RealAlloc> inline
bool operator ==(const std_allocator_adaptor<_Ty, _RealAlloc>&, const std_allocator_adaptor<_Other, _RealAlloc>&)
{
    return (true);
}

template<class _Ty, class _Other, class _RealAlloc> inline
bool operator !=(const std_allocator_adaptor<_Ty, _RealAlloc>& _Left, const std_allocator_adaptor<_Other, _RealAlloc>& _Right)
{
    return (!(_Left == _Right));
}
...
{	// defines traits for allocators
    ...
    typedef typename std::conditional<std::is_void<value_type>::value, 
...
bool operator==(const alloc_wrapper<_Ty>& _Left, const alloc_wrapper<_Other>& _Right) throw()
