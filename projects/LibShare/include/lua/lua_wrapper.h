...
struct CheckCFuncArgValid<T, Rest...> : public CheckCFuncArgValid<Rest...>
{
    //不能包含左值引用的参数
...
template<class... T>
struct CheckCFuncArgValid<std::tuple<T...>> : public CheckCFuncArgValid<T...>
{};
...
struct luaCFunctionDispatcher<CallType::POINTER_TO_FUNCTION,
                              true,
                              _CallableType,
                              IntegerSequence<index...>>
{
    ...
    static int Execute(lua_State *pLua, _PfType pfn)
    {
        ...
        pfn(lua_io_dispatcher<std::decay_t<
                typename std::tuple_element<index, typename _CallableType::arg_tuple_t>::type>>::
                              ...
                              IntegerSequence<index...>>
{
    ...
    static int Execute(lua_State *pLua, _PfType pfn)
    {
        ...
        return lua_io_dispatcher<result_type>::to_lua(
            pLua,
            pfn(lua_io_dispatcher<std::decay_t<typename std::tuple_element<
                    index,
                    typename _CallableType::arg_tuple_t>::type>>::from_lua(pLua, index + 1)...));
    }
};

//模板特化, 成员函数指针, 返回void
template<class _CallableType, size_t... index>
struct luaCFunctionDispatcher<CallType::POINTER_TO_MEMBER_FUNCTION,
                              true,
                              _CallableType,
                              IntegerSequence<index...>>
{
        ...
        if (pThis)
        {
            (pThis->*pmf)(
                lua_io_dispatcher<std::decay_t<typename std::tuple_element<
                    index,
                    typename _CallableType::arg_tuple_t>::type>>::from_lua(pLua, index + 2)...);
        }
        return 0;
    }
};
...
struct luaCFunctionDispatcher<CallType::POINTER_TO_MEMBER_DATA,
                              false,
                              _CallableType,
                              IntegerSequence<>>
{
...
//模板特化, 函数对象, 返回void
template<class _CallableType, size_t... index>
struct luaCFunctionDispatcher<CallType::FUNCTION_OBJECT,
                              true,
                              _CallableType,
                              IntegerSequence<index...>>
{
    ...
    {
        fnObj(lua_io_dispatcher<std::decay_t<
                  typename std::tuple_element<index, typename _CallableType::arg_tuple_t>::type>>::
                  from_lua(pLua, index + 1)...);
        return 0;
    }
                              ...
                              IntegerSequence<index...>>
{
    ...
    static int Execute(lua_State *pLua, _PfType &&fnObj)
    {
        ...
        return lua_io_dispatcher<result_type>::to_lua(
            pLua,
            fnObj(lua_io_dispatcher<std::decay_t<typename std::tuple_element<
                      index,
                      typename _CallableType::arg_tuple_t>::type>>::from_lua(pLua, index + 1)...));
    }
};
...
struct PushCppCallableDispatcher<false>
{
    ...
    static void PushImpl(lua_State *pLua, _CallType &&pf)
    {
        using _Call_t = std::decay_t<_CallType>;
        using _FuncObj_Store_t = std::shared_ptr<void>;
        _FuncObj_Store_t *pObj = (_FuncObj_Store_t *)::lua_newuserdata(pLua,
...
/** 向栈上压入C++调用
@param[in,out] pLua  lua_Satate指针
@param[in] pf C++ callable object, 限制: 支持"copy construct(left value)"或"move construct(right value)".
           参数限制：不支持非const的左值引用的参数(非const的左值引用的参数表示出参，以什么样的顺序push到lua栈上是不明确的).
*/
template<class _CallType>
void push_cpp_callable_to_lua(lua_State *pLua, _CallType &&pf)
{
    ::luaL_checkstack(pLua, 1, "too many upvalues");
    Internal::PushCppCallableDispatcher<std::is_trivially_destructible<
        std::decay_t<_CallType>>::value>::PushImpl(pLua, std::forward<_CallType>(pf));
}
