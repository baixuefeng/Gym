...
namespace
{
    bool compress_data_impl(const void * pInput, uint32_t nLength, std::vector<uint8_t> & output, int nLevel, int windowBits)
    {
        output.clear();
        if (!pInput || nLength == 0)
        {
        ...
        z_stream strm{ 0 };
        int err = ::deflateInit2(&strm, nLevel, Z_DEFLATED, windowBits, MAX_MEM_LEVEL, Z_DEFAULT_STRATEGY);
        if (err != Z_OK)
        {
            return false;
        }
        ...
        do
        {
            ...
            if (err == Z_OK ||
                (flushFlag == Z_FINISH && err == Z_BUF_ERROR))
            {
                if (strm.avail_out == 0)
                {
            ...
            {
                output.insert(output.end(), spBuffer.get(), spBuffer.get() + CACHE_BUFFER_SIZE - strm.avail_out);
                break;
            }
        } while (err == Z_OK);
    ...
    bool compress_data_impl(const void * pInput, uint32_t nLength, void * pOutput, uint32_t & nOutLength, int nLevel, int windowBits)
    {
        if (!pInput || nLength == 0)
        {
            return true;
