...
void GraphicText::OnReadingXmlAttribute(const pugi::xml_attribute & attr)
{
    ...
    {
        m_fontWeight = (DWRITE_FONT_WEIGHT)attr.as_int(DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_NORMAL);
    }
    ...
    {
        m_fontStretch = (DWRITE_FONT_STRETCH)attr.as_int(DWRITE_FONT_STRETCH::DWRITE_FONT_STRETCH_NORMAL);
    }
...
bool GraphicText::CreateTextLayout()
{
    ...
    HRESULT hr = pRoot->DWritePack().m_spDWriteFactory->CreateTextFormat(
        m_fontName.c_str(),
        NULL,
        m_fontWeight,
        m_fontStyle,
    ...
    if (GetLayerBounds().IsRectEmpty())
    {
        ...
        CD2DSizeF szExtra(m_textMargin.left + m_textMargin.right, m_textMargin.top + m_textMargin.bottom);
        if (m_fontStyle != DWRITE_FONT_STYLE::DWRITE_FONT_STYLE_NORMAL)
        {
            //斜体字,宽度增加一些,否则文字会被裁掉一部分
            szExtra.width += m_fontSize / 6;
        }
        rcBound.right = rcBound.left + (LONG)std::ceil(dwMx.widthIncludingTrailingWhitespace + szExtra.width);
...
void GraphicText::Paint(D2DRenderPack & d2dRender)
{
    ...
    {
        d2dRender.m_spSolidBrush->SetColor(D2D1::ColorF(m_textColor, COLOR32_GetA(m_textColor) / 255.f));
        ...
        d2dRender.m_spD2DRender->DrawTextLayout(m_textMargin.TopLeft(), m_spTextLayout, d2dRender.m_spSolidBrush, D2D1_DRAW_TEXT_OPTIONS::D2D1_DRAW_TEXT_OPTIONS_CLIP);
        ...
        if (m_nDebugLevel > 0 && !m_text.empty())
        {
            ...
            CD2DPointF ptLineEnd(m_textMargin.left + textMx.widthIncludingTrailingWhitespace, m_textMargin.top);
            ...
            for (size_t i = 0; i < lineMxs.size(); ++i)
            {
                ...
                d2dRender.m_spD2DRender->DrawLine(ptLineStart + ptBaseLine, ptLineEnd + ptBaseLine, d2dRender.m_spSolidBrush);
                ...
                if (m_nDebugLevel > 1)
                {
                    ...
                    d2dRender.m_spSolidBrush->SetColor(D2D1::ColorF(1.0f, 0, 0));
                    for (UINT32 j = 0; j < lineMxs[i].length; j += clusterMxs[nClusterIndex++].length)
                    {
                        ...
                        d2dRender.m_spD2DRender->DrawLine(ptClusterStart, ptClusterEnd, d2dRender.m_spSolidBrush);
                    }
                }
                ...
                if (i + 1 < lineMxs.size())
                {
                    d2dRender.m_spD2DRender->DrawLine(ptLineStart, ptLineEnd, d2dRender.m_spSolidBrush);
                }
            }
