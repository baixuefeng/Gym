...
struct SimpleIOCPPipeCenter::TPipeOverLapped
    : public OVERLAPPED
{
    TPipeOverLapped()
    {
...
IOCPThreadPool & SimpleIOCPPipeCenter::GetThreadPool(DWORD dwNumOfRun /*= 0*/, DWORD dwNumOfMax /*= 0*/)
{
    if (!m_threadPool)
    {
        //加锁,确保线程安全
...
HANDLE SimpleIOCPPipeCenter::CreatePipe(std::wstring & name)
{
    ...
    HANDLE hPipe = ::CreateNamedPipe(
        name.c_str(),
        PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED | FILE_FLAG_FIRST_PIPE_INSTANCE | WRITE_OWNER, 
...
bool SimpleIOCPPipeCenter::AsyncWaitForClientConnect(HANDLE hPipe, void * pUserData)
{
    ...
    if (!::ConnectNamedPipe(hPipe, pOvlp) && 
...
void SimpleIOCPPipeCenter::OnIOCompleted(DWORD dwErrno, DWORD dwNumberOfBytesTransferred, LPOVERLAPPED pOverlapped)
{
    assert(pOverlapped);
    if (!pOverlapped)
    {
...
void SimpleIOCPPipeCenter::OnIOCompletedImpl(DWORD dwErrno, DWORD dwNumberOfBytesTransferred, TPipeOverLapped* pOvlp)
{
    ...
    case TOperatationType::OP_SEND:
        ...
        OnSend(bSuccess, pOvlp->m_hPipe, pOvlp->m_pBuffer, pOvlp->m_nDataSize, pOvlp->m_nCompletedSize, pOvlp->m_pUserData);
        break;
    case TOperatationType::OP_RECEIVE:
        if (bSuccess)
        {
            ...
            else
            {
                if (pOvlp->m_nDataSize == 0)
                {
                    ...
                    pOvlp->m_nDataSize = pOvlp->m_pThis->GetDataLengthFromHeader(pOvlp->m_hPipe, pOvlp->m_pBuffer, pOvlp->m_nHeaderSize, pOvlp->m_pUserData);
                }
                //计数剩余长度
            ...
            if (nLeftSize > 0)
            {
                ...
                {
                    //缓存不够
                    bSuccess = pOvlp->m_pThis->ReallocOverLapped(pOvlp, pOvlp->m_nCompletedSize + nLeftSize);
                }
                if (bSuccess)
                {
                    //接收剩余的数据
        ...
        OnReceive(bSuccess, pOvlp->m_hPipe, pOvlp->m_pBuffer, pOvlp->m_nCompletedSize, pOvlp->m_pUserData);
        break;
    default:
        assert(0);
        break;
...
bool SimpleIOCPPipeCenter::SendBuffer::AddData(const void * pData, uint32_t nDataLength)
{
    ...
    if (m_pOvlp &&
        (m_pOvlp->m_nBufferSize - m_pOvlp->m_nDataSize >= nDataLength))
    {
