...
static void TestFunc1(int a, double b, bool c, const std::wstring &d, const RECT &e)
{
    ...
    std::wcout << a << L" " << b << L" " << std::boolalpha << c << L" " << d << L" " << L"{ "
               << e.left << ", " << e.top << ", " << e.right << ", " << e.bottom << L" }"
               << std::endl;
    std::wcout.imbue(pre);
}
...
{
    os << shr::lua_ostream::table_begin << compoment.m_a;

    shr::lua_ostream sub(os.get());
    sub << shr::lua_ostream::table_begin << compoment.m_b.left << compoment.m_b.top
        << compoment.m_b.right << compoment.m_b.bottom << shr::lua_ostream::table_end;
    os.insert_subtable(sub);

    os << compoment.m_c << shr::lua_ostream::table_end;
    return os;
}
...
struct LuaTestClass
{
    ...
    wchar_t *GetStr() { return (wchar_t *)m_str.c_str(); }

    void SetCompoment(const WrapCompoment &compoment) { m_wrapCompoment = compoment; }
...
struct StrCmp
{
    ...
    StrCmp(StrCmp &&) {}
    StrCmp &operator=(StrCmp &&) { return *this; }
