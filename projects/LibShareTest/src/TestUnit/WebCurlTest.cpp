...
void TestWebCurl()
{
    ...
    easyHandle.SetUrl(
        "http://101.44.1.124/files/A105000006DD021F/dl.2345.com/haozip/haozip_v5.9.5.exe");
    ...
    easyHandle.SetHeaderCallback([](const char *pBuffer, size_t nBytes) {
        (void)nBytes;
        tcout << L"length=" << nBytes << L"content=" << pBuffer;
    });
    easyHandle.SetWriteCallback(
        [pFile](const char *pBuffer, size_t nBytes) { std::fwrite(pBuffer, nBytes, 1, pFile); });
    easyHandle.SetProgressCallback([](int64_t nDownloadTotal,
